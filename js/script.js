...
// and switching between content sections. Dispatches events for sections that
// need to load specific data.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Get references to all potential content sections (using data-section-id for better decoupling)
    // Ensure all sections you want to manage have a unique ID and the 'info-section' class
    // and potentially 'hidden-section' initially in HTML.
    ...
    function buildSidebarMenu(role) {
        ...
        // 'home-section' is the welcome header
        const baseMenuItems = [
            { text: 'Home', target: 'home-section' },
            { text: 'Rules', target: 'rules' },
            { text: 'Credit', target: 'credit' },
            { text: 'Settings', target: 'settings' }, // Assuming Settings section has id="settings"
            { text: 'Account Management', target: 'account-management-section' } // Added for all roles
        ];
    ...
    function addSidebarLinkListeners() {
         ...
         links.forEach(link => {
             // Ensure we only add listeners to links that have a data-target (i.e., not the Logout link)
    ...
    function showSection(sectionId) {
        ...
        contentSections.forEach(section => {
         ...
         if (!targetSectionFound) {
             ...
             // Update URL hash to home section as well
        ...
        // Other scripts (like serverposts.js and logininfo.js for logs/account) listen for these
        ...
        } else if (sectionId === 'account-management-section') { // New: Account Management event
    ...
    document.addEventListener('authComplete', (event) => {
     ...
     // Use ID for robustness when checking visibility, as mainContentArea might be null if not logged in.
     ...
     if (mainContentAreaOnLoad && mainContentAreaOnLoad.style.display !== 'none') {
          ...
          // We call showSection to ensure the correct section is active based on hash on refresh.
           const initialSection = window.location.hash ? window.location.hash.substring(1) : 'home-section';
    ...
    window.addEventListener('hashchange', () => {
         ...
         if (mainContentAreaCheck && mainContentAreaCheck.style.display !== 'none') {
             ...
             // If hash changes while logged out, ensure auth area is visible and hash is cleared
             document.getElementById('authArea').style.display = 'flex';
             document.getElementById('mainContent').style.display = 'none';
              history.replaceState(null, '', window.location.pathname); // Replace hash without adding to history
         }
    });
